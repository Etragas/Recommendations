...
def build_params():
    ...
    parameters[keys_col_latents] = Variable(torch.from_numpy(scale * np.random.rand(movie_latent_size, num_movie_latents))).float()  # Column Latents
    parameters[keys_row_latents] = Variable(torch.from_numpy((scale * np.random.rand(num_user_latents, user_latent_size)))).float()  #Row Latents

    parameters[keys_rating_net] = RatingGeneratorNet()
    print(parameters[keys_rating_net])
    return parameters
