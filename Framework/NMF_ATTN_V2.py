...
NODE_MAP = {}
GEN_DEP = {}
def standard_loss(parameters, iter=0, data=None, indices=None, num_proc=1, num_batches = 1, reg_alpha = .01, predictions=None):
    """
...
class Node():
    def __init__(self,idx = -1, type = None, depth = -1):
        self.children = []
        self.parents = []
        self.type = type
        self.depth = depth
    ...
    def add_dependents(self,new_dep):
        self.dependents.append(new_dep)
    ...
    def __str__(self):
        return "Type: {}, Index: {}, D: {} \n" \
               "In Flow: {} \n" \
               "Out Flow: {}".format(self.type,self.idx,self.depth,self.children,self.parents)

    def __repr__(self):
        return str((self.type,self.idx,self.depth))

    def tuple(self):
        return (self.type,self.idx,self.depth)
...
def get_pred_for_users(parameters, data, indices=None):
    ...
    #Build out the dependency graph
    ...
    for user_index,movie_indices in indices:
        ...
        for movie_index in movie_indices:
            ...
            movie_pred_node = Node(movie_index,MOVIE,MAX_RECURSION)
            # Generate dependency graph
            gen_dependencies(data,parameters,movie_pred_node)
        #print(NODE_MAP[(USER,3,3)])
        #print(NODE_MAP[(USER,3,1)])
        #if (user_index == 14):
            #print(NODE_MAP[(USER,14,4)])
            #print("Node 14")
            #raw_input()
        #print(NODE_MAP[(USER,11,3)])
        #for k,v in NODE_MAP.iteritems():
        #    if v.latent is not None:
                #print(v)
                #print(v.latent)
...
def build_latents(data, parameters, max_depth):
    ...
    for source_depth in range(0,max_depth):
        #Do one pass where we aggregate all the inputs to the nets
        #Then do one pass with the nets and return the result
        ...
        for node_tuple in [x for x in NODE_MAP.keys() if x[2] == source_depth]:
            ...
            if node.depth != source_depth or node.latent is None:
                    ...
                    #print("Node with depth ",node.depth, "and index {} Had no latent".format(node.idx))
                continue
            #Bin dependents according to rating
            rating_bins = {}
            for dep_node ,rating in node.parents:
                if rating not in rating_bins:
                    rating_bins[rating] = []
                rating_bins[rating].append(dep_node)
            for rating in rating_bins.keys():
                    # print(node.tuple())
                    # print(rating)
                    if (node.type == USER):
                        #print(node.latent)
        ...
        for net_queue in userToMovieNetInputs, movieToUserNetInputs:
        ...
        for node_tuple in NODE_MAP.keys():
            node = NODE_MAP[node_tuple]
            if node.depth == source_depth+1:
                #print("Keys are",model_outs.keys())
                #print([(x,x in model_outs.keys()) for x in node.children])
                dense_latents = []
                for node_child in node.children:
                    if node_child in model_outs.keys():
                        dense_latents.append(model_outs[node_child])
                #print(len(dense_latents))
                intermediate_latent = np.array(dense_latents)
                #print("MEOW")
                #print(node.children)
                #print(intermediate_latent)
                if (intermediate_latent.any()):
                    node.latent = np.mean(intermediate_latent,axis=0)
        #print("Done with models")
    #print("These are the ill-defined",[(node.latent,node.tuple()) for node in NODE_MAP.itervalues() if node.idx > num_movie_latents])
    #Start at the lowest depth, and go up
    #For each level, concatenate all the latents of each type, then pass
# def print_tree(root):
#     print("This node has type {} with index {} and depth {}".format(root.type,root.idx,root.depth))
#     print("It depends on the following vectors".format(root.children))
#     print(root.children)
#     print("And the following vectors dependend on it".format(root.parents))
#     print(root.parents)
...
def gen_dependencies(data, parameters, current_node):
    ...
    user_to_movie_net_parameters = parameters[keys_user_to_movie_net]
    colLatents = parameters[keys_col_latents]
    #Check if node is at lowest possible recursion, if so, do nothing
    ...
    evidence_count = 0
    evidence_limit = EVIDENCELIMIT / (2 ** (MAX_RECURSION - current_node.depth))

    if (current_node.type == USER):
        items, ratings = get_candidate_latents(data[keys_row_first][current_node.idx][get_items], data[keys_row_first][current_node.idx][get_ratings], split=num_movie_latents)
        child_node_type = MOVIE
    else:
        items, ratings = get_candidate_latents(data[keys_col_first][current_node.idx][get_items], data[keys_col_first][current_node.idx][get_ratings], split = num_user_latents)
        child_node_type = USER
    ...
    for latent_source_idx, rating in zip(items,ratings):
        ...
        if(child_node_tuple not in NODE_MAP):
            child_node = Node(idx=latent_source_idx, type = child_node_type, depth=current_node.depth - 1)
            gen_dependencies(data,parameters,child_node)
            GEN_DEP[child_node.tuple()] = 1
        child_node = NODE_MAP[child_node_tuple]
        if current_node not in child_node.parents:
            child_node.parents.append((current_node,rating))
...
def recurrent_inference(parameters, data=None, user_index=0, movie_index=0):
    ...
    movieLatent = NODE_MAP[(MOVIE,movie_index,MAX_RECURSION)].latent
    userLatent = NODE_MAP[(USER,user_index,MAX_RECURSION)].latent
    #print("Movie latent",movie_index,movieLatent.shape)
    #print("User latent",user_index,userLatent.shape)
...
def print_perf(params, iter=0, gradient={}, train = None, test = None):
    ...
    print("Loss is ", loss(parameters=params, data=train,predictions=preds))
    if (test):
        print "TEST"
...
def wipe_caches():
    global USERLATENTCACHE, MOVIELATENTCACHE, UCANHIT, MCANHIT, UserDependenciesMap, MovieDependenciesMap, NODE_MAP, GEN_DEP
    global hitcount
    NODE_MAP = {}
    GEN_DEP = {}
    UserDependenciesMap = {}
