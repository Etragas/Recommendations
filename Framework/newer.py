...
def black_adam(grad_funs, init_params, callback=None, num_iters=100,
    ...
    multi_grad, multi_flat, multi_x = [], [], []
    for proc_num in range(num_proc):
        flattened_grad, unflatten, x = flatten_func(grad_funs[proc_num], init_params)
        multi_grad.append(flattened_grad)
        multi_flat.append(unflatten)
        multi_x.append(x)

    m = np.zeros(len(multi_x[0]))
    v = np.zeros(len(multi_x[0]))
...
def neural_net_inference(parameters,iter = 0, data = None):
    ...
    print num_rows
    half1 = get_pred_for_users(parameters,data,np.array(range(num_rows)))
    wipe_caches()
    return half1
