...
import multiprocessing as mp_vanilla
from multiprocessing.queues import Queue
curtime = 0
MAX_RECURSION = 100

movie_cache_lock = mp_vanilla.Lock()
user_cache_lock = mp_vanilla.Lock()
rowLatents = 0
colLatents = 0
hitcount = 0
...
def neural_net_inference(parameters,iter = 0, data = None):
    ...
    for x in range(num_cpu) :
        ...
        prev_pos += ranges[x]
        pool.apply_async(get_pred_for_users, args=(parameters,data,data_range,result_queue))
    pool.close()
    pool.join()
    results = [result_queue.get() for x in range(num_cpu)]

    sorts = sorted(results,key = lambda tup: tup[0])
