...
def get_predictions_tensor(parameters, data: non_zero_hero, indices=None):
    ...
    for userIdx, itemIdx in indices:
        ...
        userLatent = getUserEmbedding(parameters, data, userIdx)[0]
        if (userLatent is None or itemLatent is None):
            full_predictions = torch.cat((full_predictions, torch.tensor([float(3.4)], requires_grad=True).type(
        ...
        if userIdx not in caller_id_with_self[0]:
            ...
            if user_latent is not None:
                latentWithRating = torch.cat(
                    ...
                    dim=0)
                userEmbeddings.append(latentWithRating)  # We got another user latent
                evidenceCount += 1
                dist = min(dist, curr_depth)
